...
Step 2: Plan is implemented through Batch 3 Part 1.
...
Step 2 also now emits stronger dependency, conflict-zone, test-obligation, and parallelization modeling, keeps carried-forward concern mapping visible on ready and blocked runs, and can optionally write internal planning debug artifacts behind `FORGE_PLAN_DEBUG=1` without changing the public `plan.json` or `plan-report.md` contract. Batch 3 Part 1 adds a bounded internal planning-assist seam that can tighten wording without changing deterministic structure, removes stale “later Step 2” report/boundary language, and adds a dedicated freeze-criteria suite plus stronger smoke coverage for warning-grade planning runs.
