...
### Added
- **`forgecli` bin** alongside `forge`, so `npx`/`npm exec` can run the CLI even when another global `forge` (older `@forgecli/forge` or another tool) shadows `forge` on PATH.

## [1.0.3] - 2026-04-21

### Fixed
- CLI `--version` could report `1.0.0` while the npm package was newer: load `package.json` via `createRequire` from `dist/src/cli.js`, and remove stale top-level `dist/*.js` artifacts by cleaning `dist/` before each build.
